...
# @Name    : test_statisticAnalysis_getTypeStatistic
...
class Test_Analysis():
    ...
    @allure.story('告警类型统计图')
    @allure.severity('critical')
    @pytest.mark.parametrize('dayCount',D.dayCount())
    @pytest.mark.parametrize('buildingId',D.buildingId())
    def test_getTypeStatistic_pass(self, request_init, assert_init, params_init,dayCount,buildingId):
        """
        用例描述：告警类型统计图通过不同的天数进行搜索统计
    ...
    def test_getTypeStatistic_data_s_pass(self, request_init, assert_init, params_init,dayCount,start,end):
        """
        用例描述：告警类型统计图通过搜索日期进行搜索统计
