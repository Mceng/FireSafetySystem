...
# @Name    : test_alarmManager_getDealFrom
...
class Test_AlarmManager():
    ...
    @allure.story('获取监测点处理表单')
    @allure.severity('critical')
    @pytest.mark.parametrize('type',D.get_type())
    @pytest.mark.parametrize('dealType',D.get_DealFrom_dealType())
    def test_alarmManager_getDealFrom_pass(self, request_init, assert_init, params_init,type,dealType):
        """
        用例描述：获取监测点全部处理表单正向用例
